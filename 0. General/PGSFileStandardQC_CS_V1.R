...
out  <- subset(df,select=c("rsid","CHR","EA", "NEA", "BETA","P"))
colnames(out) <- c("SNP","CHR", "A1", "A2", "BETA","P")

# 导出所有染色体
if (grepl("CKB",outdir,fix=TRUE,ignore.case=TRUE)){

    fwrite(out[,-2],paste0(PGS_FILE,".CKB.CS.QC"),sep=" ",quote=F,na="NA")

}else{

    fwrite(out[,-2],paste0(PGS_FILE,".UKB.CS.QC"),sep=" ",quote=F,na="NA")
}
message(PGS_FILE,".CS.QC is stored")
...
for (i in 1:22) {
    ...
    if (nrow(outchr)>0) {
        fwrite(outchr[,-2], file=paste0("chr", i, ".CS.txt"), sep=" ", quote=F, na="NA")
    }
}
