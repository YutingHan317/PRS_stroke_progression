...
*** 有些变量感觉没啥用，删掉
drop tier ad_date dis_date is_outpatient type type2 datedeveloped source_desc 
...
* 首次卒中类型:这里给四种亚型一个优先级，SAH>HS>IS>Unspecified
...
save "1.Data/sec_stroke.dta",replace

********************************************************************* 与随访数据库合并
use "1.Data/full_data.dta",clear
merge 1:1 csid using "1.Data/first_stroke.dta",nogen keep(master match)
merge 1:1 csid using "1.Data/sec_stroke.dta",nogen keep(master match)
* 检查一下生成的结局变量是否有误
...
export delimited "1.Data/cleaned_recur_stroke.csv", nolabel datafmt replace
